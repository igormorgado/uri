        ...
        for (size_t i = 0; i < 2; ++i)
        {
            if ((lower_bound < roots[i]) && (roots[i] < upper_bound))
            {
                ...
                if (y > max_vol)
                {
                    max_vol = y;
                    max_x = roots[i];
                    // printf("Root[%zu] = %.3f found. With value %.3f\n",
                    //         i, max_x, max_vol);
                }
            }
            // else
            // {
            //     printf("Root[%zu] = %.3f ignored out of bound (%.3f, %.3f)\n",
            //             i, roots[i], lower_bound, upper_bound);
            // }
        }
        //printf("W: %7.3f H: %7.3f x0: %7.3f x1: %7.3f  df(x0): %7.3f  df(x1): %7.3f f(x0): %7.3f f(x1): %7.3f\n",
        //        W, H, roots[0], roots[1], df(roots[0], W, H), df(roots[1], W, H), f(roots[0], W, H), f(roots[1], W, H));
