...
orientation(Vec2 v0, Vec2 v1, Vec2 v2)
{
    ...
    if      (s > 0) return ORI_COUNTERCLOCKWISE;
    else if (s < 0) return ORI_CLOCKWISE;
    else            return ORI_COLINEAR;
}
...
__cmpcmpt(const void *p1, const void *p2)
{
    ...
    if          (_p1.a < _p2.a)  return -1;
    else if     (_p1.a > _p2.a)  return  1;
    else
...
int main(void) {
    Vec2 *V = malloc (2000 * sizeof *V);
    Vec2 *CH = malloc (2000 * sizeof *CH);
    int N = 0;
    while(scanf("%d\n", &N), N!=0)
    {
        for(size_t i = 0; i < N; ++i)
